...
export default function Projects() {
  ...
  return (
    <section>
              ...
              <Descriptions
                ...
                data={[
                  // { key: '满意度', value: <Rating allowHalf size="small" value={item.rating} /> },
                  {
                    key: 'DEV环境：',
                    value: (
                      <a target="_blank" href={'https://magic.tarsocial.com/'}>
                        {item.devUrl}
                      </a>
                    ),
                  },
                  item.uatUrl
                    ? {
                        key: 'UAT环境：',
                        value: (
                          <a target="_blank" href={item?.devUrl}>
                            {item.uatUrl}
                          </a>
                        ),
                      }
      ...
      />
      <Modal title="新增项目" visible={visible} onOk={addProject} onCancel={() => setVisible(false)} closeOnEsc={true}>
        <Form labelPosition="left" labelAlign="right" getFormApi={(formApi) => (api.current = formApi)}>
          <Input
            label={'项目名称'}
            field={'name'}
            trigger="blur"
          ...
          />
          <Input label={'DEV地址'} field="devUrl" className="form-item" stopValidateWithError rules={[]} />
          <Input label={'UAT地址'} field="uatUrl" className="form-item" stopValidateWithError rules={[]} />
          <Input label={'PROD地址'} field="prodUrl" className="form-item" stopValidateWithError rules={[]} />
          <Input label={'项目描述'} field="description" className="form-item" stopValidateWithError />
        </Form>
      </Modal>
    </section>
  );
